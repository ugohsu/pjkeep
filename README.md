...
- ユーザー/パスワード認証。admin がプロジェクト単位で read（閲覧）/ write（編集）権限を管理でき、複数ユーザーによる共同作業にも対応
- 複式簿記による仕訳入力・仕訳帳・損益計算書・貸借対照表
- ダッシュボード：比例縮尺損益計算書・純資産推移グラフの固定ウィジェット＋費用科目ごとの予算実績比較グラフ（複数追加可）
- 総勘定元帳（科目別残高推移・相手科目表示・仕訳編集・TSV エクスポート）
